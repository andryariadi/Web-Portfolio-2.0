    ...
    <motion.main className="h-full" initial={{ y: "-200vh" }} animate={{ y: "0%" }} transition={{ duration: 1 }}>
      <div className="h-full flex flex-col lg:items-center lg:justify-center lg:flex-row gap-3 lg:gap-5 px-4 sm:px-8 md:px-12 lg:px-20 xl:px-48 py-5 ">
        ...
        <div className="h-[70%] lg:h-full lg:w-[71%] flex flex-col items-center justify-center gap-3 md:gap-8 lg:gap-8 text-black">
          <motion.h1 variants={fadeIn("up", 0.3)} initial="hidden" whileInView={"show"} viewport={{ once: false, amount: 0.7 }} className="text-slate-900 text-3xl sm:text-4xl md:text-6xl 2xl:text-6xl font-orbitron font-bold w-full">
          ...
          <motion.div variants={fadeIn("up", 0.5)} initial="hidden" whileInView={"show"} viewport={{ once: false, amount: 0.7 }} className="flex items-center gap-2 w-full font-aldrich font-semibold uppercase leading-[1]">
            <h2 className="text-slate-900 text-lg sm:text-xl md:text-4xl 2xl:text-4xl">I am a</h2>
            <TypeAnimation sequence={["Software Engineering", 2000, "Web Developer", 2000, "Frontend Developer", 2000]} speed={50} wrapper="span" repeat={Infinity} className="text-[#739248] text-lg sm:text-xl md:text-4xl 2xl:text-4xl" />
          </motion.div>
          <motion.p className="w-full font-semibold font-rajdhani text-lg">Welcome to My personal website</motion.p>
          <motion.div variants={fadeIn("up", 0.7)} initial="hidden" whileInView={"show"} viewport={{ once: false, amount: 0.7 }} className="w-full flex gap-5">
            <Link href="https://drive.google.com/file/d/1MXqH30qnaYmYNHtPij0KXB2oPJoecwjV/view?usp=sharing" target="_blank" className="w-fit font-semibold p-2 md:p-3 rounded-lg ring ring-slate-900 bg-slate-900 text-white font-aldrich">
